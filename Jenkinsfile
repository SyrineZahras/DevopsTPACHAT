    ...
    stages {
        ...
        stage('MVN COMPILE') { 
         ...
         stage('MVN SONARQUBE') {
                 ...
                 stage ('NEXUS DEPLOY') {
                     steps {
                         sh 'mvn clean package deploy:deploy-file -DgroupId=com.esprit.examen -DartifactId=tpAchatProject -Dversion=1.0 -DgeneratePom=true -Dpackaging=jar -DrepositoryId=deploymentRepo -Durl=http://192.168.16.73:8081/repository/maven-releases/ -Dfile=target/tpAchatProject-1.0.jar -DskipTests'
                     }
                 }
